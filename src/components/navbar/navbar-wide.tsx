...
export default function Navbar () {
  ...
  return <>
    <div className="w-[40vw] mr-auto max-[800px]:w-[35vw]">
    </div>
    <nav className='w-[40vw] max-[800px]:w-[35vw] h-screen fixed top-0 left-0 flex flex-col'>
      ...
      <ul className='text-2xl tracking-widest flex flex-col gap-y-1 m-auto [&>li]:cursor-pointer'>
        {
          navEntrys.map(el => <NavEntry isCurrent={router.asPath === el.url} url={el.url} name={el.name} key={el.name} />)
        }
        {
          Boolean(admin) && <>
            <NavEntry url='/admin' isCurrent={router.asPath === '/admin'} name={'Admin'} />
