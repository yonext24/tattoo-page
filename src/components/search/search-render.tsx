...
export function SearchRender ({ error, clientError, loading, tattoos, intersected }: Props) {
  return <>
    {
      (error !== undefined || (Boolean(clientError)))
        ? <span className='text-center'>{error ?? clientError}</span>
