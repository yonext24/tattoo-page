...
export default function DesignUploadPage () {
  const { submitHandler, precioHandler, uploadImageHandler, state, fileInputRef } = useUploadDesign()
  ...
  <Seo title='Designs / Neptuno Black Tattoos' />
    <main className='flex-1 h-max overflow-y-hidden pr-2 flex flex-col min-h-screen items-center max-w-xl pl-2 relative
      ...
      <form onSubmit={submitHandler} className='flex-1 flex flex-col items-center mx-auto relatie'>
        ...
        <DesignAdminInputs design={design} fetch={fetch} precioHandler={precioHandler} />
        {
          fetch.error
      ...
      ${fetch.error ? 'bg-red-400/70 pointer-events-auto' : ''}`}>
        ...
        {
          fetch.success && <span className='text-black text-xl font-bold text-center'>El diseño se subió correctamente.</span>
        }
        {
