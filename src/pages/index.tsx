    ...
    <>
      ...
      <main className="flex-1 flex flex-col align-center px-6 h-max max-w-xl overflow-x-hidden max-[630px]:max-w-none">
        <section className="h-screen w-full flex items-end justify-center flex-col max-[630px]:items-center">
          <div
            className={
              'transition-[opacity,transform] duration-500 ease-out relative flex items-center'
            }
          >
            <div className={'animate-fadeTop transition-all duration-500'}>
              <Glitch
                text="Alan Hernandez"
                className={
                  'text-4xl self-center title text-gold [writing-mode:vertical-lr] rotate-180 top-[.28rem] left-[.15rem]'
                }
              >
                <h2 className="text-4xl title [writing-mode:vertical-lr] rotate-180">
                  Alan Hernandez
                </h2>
              </Glitch>
            </div>
            <Outline>
              <Image
                ...
                height={500}
                width={401}
                className="h-full rounded-[inherit] object-cover object-top"
              />
            </Outline>
          </div>
          <div
            className={
              'animate-fadeRight duration-500 ease-out transition-transform grid grid-cols-2 gap-4 mt-2 w-[calc(100%-40px)] max-w-[410px] max-[630px]:pl-[40px]'
            }
          >
            {medias.map((el) => (
              <MediaCard key={el.name} name={el.name} url={el.url} />
            ))}
          </div>
        </section>
        <About />
        <a
          className="text-xs group text-end text-neutral-500 -mb-5"
          href="https://yonathan-portfolio.netlify.app/"
          target="_blank"
...
Home.getLayout = (page: ReactNode): ReactNode => <Layout>{page}</Layout>
